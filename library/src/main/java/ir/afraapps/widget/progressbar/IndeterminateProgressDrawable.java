/*
 * Copyright (c) 2015 Zhang Hai <Dreaming.in.Code.ZH@Gmail.com>
 * All Rights Reserved.
 */

package ir.afraapps.widget.progressbar;

import android.animation.Animator;
import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.graphics.RectF;

import androidx.annotation.Keep;


public class IndeterminateProgressDrawable extends BaseIndeterminateProgressDrawable {


  private static final int PROGRESS_INTRINSIC_SIZE_DP = 42;
  private static final int PADDED_INTRINSIC_SIZE_DP = 48;
  private static final RectF RECT_BOUND = new RectF(-21, -21, 21, 21);
  private static final RectF RECT_PADDED_BOUND = new RectF(-24, -24, 24, 24);
  private static final RectF RECT_PROGRESS = new RectF(-19, -19, 19, 19);

  private int mProgressIntrinsicSize;
  private int mPaddedIntrinsicSize;
  private float thickness = 3;

  private boolean smoothProgress;
  private boolean indeterminate;

  private float progress;
  private float progressFrom;

  private Animator animIndeterminate;
  private Animator animIndeterminateRotation;

  private RingPathTransform mRingPathTransform = new RingPathTransform();
  private RingRotation mRingRotation = new RingRotation();


  /**
   * Create a new {@code IndeterminateProgressDrawable}.
   *
   * @param context the {@code Context} for retrieving style information.
   */
  public IndeterminateProgressDrawable(Context context, float thickness, boolean indeterminate,
                                       float progress, boolean smoothProgress) {
    super(context);

    float density = context.getResources().getDisplayMetrics().density;
    mProgressIntrinsicSize = Math.round(PROGRESS_INTRINSIC_SIZE_DP * density);
    mPaddedIntrinsicSize = Math.round(PADDED_INTRINSIC_SIZE_DP * density);

    this.smoothProgress = smoothProgress;
    this.indeterminate = indeterminate;
    this.progress = progress;
    this.progressFrom = 0;
    if (thickness > 0) {
      this.thickness = thickness;
    }
    prepare();

      /*  mAnimators = new Animator[]{
                Animators.createIndeterminate(mRingPathTransform, indeterminateOnly),
                // Animators.createIndeterminateRotation(mRingRotation)
        };*/

  }


  private void prepare() {

    if (indeterminate) {
      animIndeterminate = Animators.createIndeterminate(mRingPathTransform);
      animIndeterminateRotation = Animators.createIndeterminateRotation(mRingRotation);
      mAnimators.add(animIndeterminate);
      mAnimators.add(animIndeterminateRotation);

    } else {

      if (smoothProgress) {
        if (animRotationIsRunning()) {
          animIndeterminate = Animators.createSmoothProgress(
            mRingPathTransform, progressFrom, progress);
          animIndeterminate.start();

        } else {
          mRingPathTransform.mTrimPathStart = 0;
          mRingPathTransform.mTrimPathOffset = 0;
          animIndeterminate = Animators.createSmoothProgress(
            mRingPathTransform, progressFrom, progress);
          animIndeterminate.start();
          if (progress > 0) {
            animIndeterminateRotation = Animators.createIndeterminateRotation(mRingRotation, mRingRotation.mRotation);
            mAnimators.add(animIndeterminateRotation);
            start();
          }

        }


      } else {
        mRingPathTransform.mTrimPathEnd = progress;
      }

    }

  }


  private boolean animRotationIsRunning() {
    return animIndeterminateRotation != null && animIndeterminateRotation.isRunning();
  }

  private int getIntrinsicSize() {
    return mUseIntrinsicPadding ? mPaddedIntrinsicSize : mProgressIntrinsicSize;
  }

  /**
   * {@inheritDoc}
   */
  @Override
  public int getIntrinsicWidth() {
    return getIntrinsicSize();
  }

  /**
   * {@inheritDoc}
   */
  @Override
  public int getIntrinsicHeight() {
    return getIntrinsicSize();
  }

  @Override
  protected void onPreparePaint(Paint paint) {
    paint.setStyle(Paint.Style.STROKE);
    paint.setStrokeWidth(thickness);
    paint.setStrokeCap(Paint.Cap.ROUND);
    paint.setStrokeJoin(Paint.Join.ROUND);
  }

  @Override
  protected void onDraw(Canvas canvas, int width, int height, Paint paint) {

    if (mUseIntrinsicPadding) {
      canvas.scale(width / RECT_PADDED_BOUND.width(), height / RECT_PADDED_BOUND.height());
      canvas.translate(RECT_PADDED_BOUND.width() / 2, RECT_PADDED_BOUND.height() / 2);
    } else {
      canvas.scale(width / RECT_BOUND.width(), height / RECT_BOUND.height());
      canvas.translate(RECT_BOUND.width() / 2, RECT_BOUND.height() / 2);
    }

    drawRing(canvas, paint);
  }

  private void drawRing(Canvas canvas, Paint paint) {

    int saveCount = canvas.save();
    canvas.rotate(mRingRotation.mRotation);

    // startAngle starts at 3 o'clock on a watch.
    float startAngle = -90 + 360 * (mRingPathTransform.mTrimPathOffset
      + mRingPathTransform.mTrimPathStart);
    float sweepAngle = 360 * (mRingPathTransform.mTrimPathEnd
      - mRingPathTransform.mTrimPathStart);
    canvas.drawArc(RECT_PROGRESS, startAngle, sweepAngle, false, paint);

    canvas.restoreToCount(saveCount);
  }


  public void setProgress(float progress) {
    if (animIndeterminate != null && animIndeterminate.isRunning()) {
      animIndeterminate.end();
    }
    progressFrom = this.progress;
    this.progress = progress;
    prepare();
  }


  public void setIndeterminate(boolean indeterminate) {
    this.indeterminate = indeterminate;
    reset();
    prepare();
  }

  private void reset() {
    end();
    if (animIndeterminate != null && animIndeterminate.isRunning()) {
      animIndeterminate.end();
    }
    progress = 0;
    progressFrom = 0;
    mRingRotation.mRotation = 0;
    mRingPathTransform.mTrimPathStart = 0;
    mRingPathTransform.mTrimPathEnd = 0;
    mRingPathTransform.mTrimPathOffset = 0;
  }


  public void setSmoothProgress(boolean smoothProgress) {
    this.smoothProgress = smoothProgress;
  }

  private static class RingPathTransform {

    public float mTrimPathStart;
    public float mTrimPathEnd;
    public float mTrimPathOffset;

    @Keep
    @SuppressWarnings("unused")
    public void setTrimPathStart(float trimPathStart) {
      mTrimPathStart = trimPathStart;
    }

    @Keep
    @SuppressWarnings("unused")
    public void setTrimPathEnd(float trimPathEnd) {
      mTrimPathEnd = trimPathEnd;
    }

    @Keep
    @SuppressWarnings("unused")
    public void setTrimPathOffset(float trimPathOffset) {
      mTrimPathOffset = trimPathOffset;
    }
  }

  private static class RingRotation {

    public float mRotation;

    @Keep
    @SuppressWarnings("unused")
    public void setRotation(float rotation) {
      mRotation = rotation;
    }
  }
}
